...
One-shot runner: predict (6) → evoskill (7) → knowledge graph (8) → skill doctor (10) → recall rebuild (11) → fleet (12) → arxiv (13) → anomaly (14) → investigate (15) → heal (16) → heal-learn (17) → unlearn (18) → family (19) → prediction audit (20) → investigator audit (22) → healer audit (23) → recall rebuild → KG → **history append → delta compare (21)**. Writes `scripts/delta_report.json`. Exit code 2 = anomaly (P>0.7) → run system-investigation protocol. Exit code 3 = unrepairable skill defects. Regression suite: `scripts/test_delta.py` replays every production misdiagnosis (Thai-month attribution, wrong-window evidence, window-wide disconfirm, desktop-subsystem faults, healer KEYMAP collisions, benign-lifecycle misconfirms, case-variant signature splits, seed-wording overclaims, self-test pollution — 37 cases) — run it after any investigator/anomaly_watch/healer change.
