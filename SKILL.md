...
Module 20 scores the PREDICTOR against future journal truth; module 22 scores the INVESTIGATOR the same way — the last un-audited component of the pipeline. An investigator that is never scored keeps misdiagnosing with the same buggy seeds forever. Scoring rules (journal = ground truth, checked AFTER the verdict's 90-min window so the future has actually happened): CONFIRMED desktop-noise verdicts are TRUE_POSITIVE if the pattern did NOT recur (transient as diagnosed), FALSE_POSITIVE if it recurred ≥2x; real-service-defect verdicts are TRUE_POSITIVE only if a real failure signature (Failed with result / main process exited / coredump / segfault) appears in the window — a benign lifecycle message misread as a defect scores FALSE_POSITIVE; desktop-subsystem-fault verdicts are TRUE_POSITIVE if the component error recurs; INSUFFICIENT_EVIDENCE verdicts are TRUE_NEGATIVE if nothing recurs, FALSE_NEGATIVE if it does (evidence existed but wasn't found). Outcomes append as `type=investigator_outcome` with `patterns_found` (evoskill materializes a skill per misdiagnosis class) AND symptoms+root_cause (recall surfaces past verdict failures). Calibration per verdict class; ≥3 scored with accuracy <50% = MISDIAGNOSING → recommendation to fix that seed (human gate — never edits auto_investigator.py). State: `scripts/investigator_audit_state.json`. Exit 2 = MISDIAGNOSING. Verified live on first run: 17 verdicts scored — real_service_defect 0% accuracy (2/2 FALSE_POSITIVE: update-notifier "starting/finished" lifecycle messages misconfirmed as defects — bug 8), desktop_noise 88% (1 FALSE_POSITIVE: tracker "transient" claim while the pattern recurred 3x), insufficient_evidence 100% (7 TRUE_NEGATIVE). Evoskill materialized 4 skills in the same run (`evolved-investigator-false_positive-real-service-defect`, `evolved-investigator-true_positive-desktop-noise`, `evolved-investigator-true_negative-insufficient-evidence`, `evolved-belief-revision-retracted-real-defect`); second run idempotent (0 re-score, 0 re-retract).
...
- **regression fixtures**: test artifacts moved to `scripts/test_fixtures/` (git-tracked) — `scripts/artifacts/` is a 20-file rotation buffer that pruned the originals mid-session and broke the suite. Suite now 46 cases ALL PASS.
